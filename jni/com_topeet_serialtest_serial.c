 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_com_topeet_serialtest_serial_Open(JNIEnv *env,
		jobject obj, jint Port, jint Rate) {
	if (fd <= 0) {
		if (0 == Port) {
			__android_log_print(ANDROID_LOG_INFO, "serial",
					"open fd /dev/ttySAC0");
...
//			O_RDONLY | O_NOCTTY | O_NDELAY
			...
			__android_log_print(ANDROID_LOG_INFO, "serial",
					"Parameter Error serial not found");
			fd = 0;
			return -1;
		}
#if 1	
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_com_topeet_serialtest_serial_Close(JNIEnv *env,
		jobject obj) {
	if (fd > 0)
		close(fd);
}
 ...
 * Signature: ()[I
 */
JNIEXPORT jbyteArray JNICALL Java_com_topeet_serialtest_serial_Read(JNIEnv *env,
		jobject obj) {
	unsigned char buffer[512];
	int BufToJava[512];
	int len = 0, i = 0;

	unsigned char buffer_back[ARRAY_LENGTH];
	...
	if (len <= 0)
		...
		__android_log_print(ANDROID_LOG_INFO, "serial", "read==>%d", len);
//		for (i = 0; i < len; i++) {
//			printf("%x", buffer[i]);
 ...
 * Signature: ()[I
 */
JNIEXPORT jint JNICALL Java_com_topeet_serialtest_serial_Write(JNIEnv *env,
		jobject obj, jintArray buf, jint buflen) {
	jsize len = buflen;
...
JNIEXPORT jbyteArray JNICALL Java_com_topeet_serialtest_serial_GetRawData(
	...
	int read_num, readcount = 0;

	unsigned char buffer[bufferlength];
	// define buffer_return to native java class
	unsigned char desc_buffer[ARRAY_LENGTH];
	memset(buffer, 0, sizeof(buffer));
	memset(desc_buffer, 0, ARRAY_LENGTH);

//	while (readcount<ARRAY_LENGTH) {
//		len = read(fd, buffer, bufferlength);
//		if (len > 0){
//			if(len+readcount<ARRAY_LENGTH)
//				memcpy(&desc_buffer[readcount],buffer,len);
//			else
//				break;
//			readcount+=len;
//		}
...
//	}
	...
	while (readcount < ARRAY_LENGTH) {
		...
		if (read_num > 0) {
			if (read_num + readcount < ARRAY_LENGTH)
				memcpy(&desc_buffer[readcount], buffer, read_num);
			else
				break;
			readcount += read_num;
		}
	}
	jbyteArray array = (*env)->NewByteArray(env, ARRAY_LENGTH);
	(*env)->SetByteArrayRegion(env, array, 0, ARRAY_LENGTH,
			(jbyte*) desc_buffer);
